...
import ThothGUI.thoth_lite.subwindows.IdentifiableCardWindow;
...
//        identifiableElementList.itemsProperty().bind( datas );
    ...
    private Button getButton(
        ...
        if(url == null) {
            res = ThothGUI.thoth_lite.components.controls.Button.getInstance(id.id, event);
        }else{
            res = ThothGUI.thoth_lite.components.controls.Button.getInstance(
                    ThothGUI.thoth_lite.components.controls.ImageView.getInstance( url, 30, 30 ),
                    event
            );
        }
